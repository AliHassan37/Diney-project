...
exports.update = tryCatchAsync(async (req, res) => {
  ...
  current_user.role = req.body.role;
  current_user.gender = req.body.gender;
  current_user.email = req.body.email;
  current_user.dob = req.body.dob;
  current_user.age = req.body.age;
  current_user.star = req.body.star;
  current_user.zodiac = req.body.zodiac;
  current_user.planet = req.body.planet;
  current_user.hair = req.body.hair;
  current_user.eyes = req.body.eyes;
  current_user.height = req.body.height;
  current_user.weight = req.body.weight;
  current_user.blood = req.body.blood;
  current_user.status = req.body.status;
  current_user.country = req.body.phone?.data?.countryCode;
...
exports.login = tryCatchAsync(async (req, res, next) => {
  ...
  if (!name && !password) {
    return next(
      new AppError("Name and password field is required", unprocessable)
    );
  }
  if (!name) {
    return next(new AppError("Name  field is required", badRequest));
  }
  if (!password) {
    return next(new AppError("Password  field is required", badRequest));
  }
  const current_user = await User.findOne({ name }).select("+password");
  if (current_user) {
    if (await current_user.isCorrectPassword(password, current_user.password)) {
...
exports.delete = tryCatchAsync(async (req, res, next) => {
  ...
  if (await user.isCorrectPassword(password, user.password)) {
    ...
    await registeredUsers.deleteOne({ email });
  ...
  return next(new AppError("Incorrect password", badRequest));
});
...
exports.music = tryCatchAsync(async (req, res, next) => {
  ...
  current_user.music = req.body.name;
